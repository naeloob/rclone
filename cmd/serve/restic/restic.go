...
func (s *server) postObject(w http.ResponseWriter, r *http.Request, remote string) {
	// We can overwrite the config object, but nothing else
	overwriteOK := strings.HasSuffix(r.URL.Path, "/config")
	if !overwriteOK {
		// The object mustn't already exist
		_, err := s.f.NewObject(remote)
		if err == nil {
			fs.Errorf(remote, "Post request: object already exists")
